<script lang="ts">
	...
	import { isDarkMode, toggleTheme } from '$lib/utils/helpers';
	...
	const handleOutsideClick = (e: MouseEvent) => {
		...
		if (!anchorEl?.contains(e.target as Node) && !drawerEl?.contains(e.target as Node)) {
			handleClose();
		}
	};
...
{#if open}
	...
	>
		<button class="mx-auto py-4" on:click|stopPropagation={handleClose}
			><Icon type="thickChevronDown" fill={iconColor} width={iconSize} heigth={iconSize} /></button
		>
		<ul
		...
		>
			<li class="">
				<a href="/plants" class="inline-flex items-center">
					...
					<Icon type="grid" fill={iconColor} width={iconSize} height={iconSize} />
				</a>
			</li>
			<li>
				<a href="/learn/light" class="inline-flex items-center">
					<p class="mr-3">Learn</p>
					<Icon type="eye" fill={iconColor} width={iconSize} height={iconSize} />
				</a>
			</li>
			<li>
				<button on:click={handleThemeToggle} class="inline-flex items-center">
					...
					{#if isDark}
						<Icon type="thinSun" fill={iconColor} width={iconSize} height={iconSize} />
					{:else}
						<Icon type="thinMoon" fill={iconColor} width={iconSize} height={iconSize} />
					{/if}
				</button>
			</li>
		</ul>
