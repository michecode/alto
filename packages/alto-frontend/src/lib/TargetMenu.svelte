<script lang="ts">
	...
	const handleOutsideClick = (event: MouseEvent) => {
		...
		if (!anchorEl?.contains(event.target as Node) && !menuEl?.contains(event.target as Node)) {
			handleClose();
		}
	};
	...
	{#if open}
      ...
      drop-shadow-lg p-4 font-serif font-bold text-2xl rounded-xl opacity-95"
		...
		>
			...
			<li role="menuitem" tabindex="-1">
				<a href="/learn/light" class="inline-flex items-center hover:opacity-70">
					<Icon type="eye" fill={iconColor} width="24px" height="24px" />
					<p class="ml-1">Learn</p>
				</a>
			</li>
			<li role="menuitem" tabindex="-1">
				<button on:click={handleThemeToggle} class="inline-flex items-center hover:opacity-70">
					{#if isDark}
